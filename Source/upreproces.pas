...
interface
uses
  Classes, SysUtils, Fgl, uPreBasicos, Dialogs, FileUtil, Forms, LCLType, Globales, DateUtils;

procedure PreProcesar(arc: String; txt: String; var ArcSal: String; var cadcon0: string);
function GenNombSal(nomBase: String; ext: string = 'sql'): String; //utilidad para manejo de nombres
...
begin
   ...
   if dirSal = '' then begin
      ShowMessage('No se ha iniciado directorio de salida');
      exit;
   end;
...
End;
procedure PreProcesar(arc: String; txt: String; var ArcSal: String; var cadcon0: string);
{Realiza el preprocesamiento de un archivo o un texto. ArcSal es el archivo de
 salida donde se escribirá el texto preprocesado. Si ArcSal está en blanco, se
 le asigna un nombre automáticamente. Si "txt" <> "", se procesa "txt", de otra
 forma procesa el archivo "arc".
...
begin
   Perr.IniError;
   cadcon0 := '';
   //Verifica si hay nombre de archivo de salida
   If arcsal = '' Then
      arcsal := GenNombSal(arc)     //lo genera
   Else begin   //Se ha especificado nombre
     If FileExists(arcsal) Then begin  //existe
        If FileIsWritable(arcsal) Then
            DeleteFile(arcsal)
        Else begin { TODO : Debería dar error si no puede generar el nombre solicitado }
            arcsal := GenNombSal(arc);     //lo genera
            //GenError 1, "No se puede lanzar consulta. Ya hay una consulta en Proceso"
            //Call VerificarError
            //Exit Sub
        End;
     End;
...
initialization
   ...
   //asigna ruta de salida
   dirSal := ExtractFilePath(Application.ExeName) + 'bolsa\';
   if not DirectoryExists(dirSal) then begin
      Application.MessageBox(PChar('No se encontró directorio: ' + dirSal + '. Se creará. '),
           '', MB_ICONEXCLAMATION);
      CreateDir(dirSal);
   end;
